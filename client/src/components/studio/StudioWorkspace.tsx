...
import { ArrowUpRight, ChevronDown, Clapperboard, Download, Image as ImageIcon, LoaderCircle, PanelLeft, Send, Settings2, Sparkles, Trash2, Upload, X } from "lucide-react";
    ...
    <section className="mx-auto flex w-full max-w-[1180px] flex-1 flex-col px-5 pb-9 pt-8 sm:px-8 lg:px-12">
      ...
      <div className="mt-8 grid flex-1 gap-5 lg:grid-cols-[minmax(0,1fr)_310px]">
        <div className="flex min-h-[520px] flex-col rounded-[26px] border border-white/[.08] bg-[#0e1012] p-4 sm:p-6">
          <div className="flex items-center justify-between border-b border-white/[.07] pb-4"><div><div className="text-sm font-bold">{props.mode === "chat" ? "Conversation" : props.mode === "image" ? "Image direction" : "Motion direction"}</div><div className="mt-1 text-xs text-white/38">{props.mode === "chat" ? "Clear context in, useful answers out." : props.reference ? "The selected image will be used as the generation reference." : "Describe the scene. Atlas handles the render."}</div></div><button onClick={props.onClear} className="rounded-lg p-2 text-white/30"><Trash2 size={15} /></button></div>

          <div className="scroll-thin flex-1 space-y-5 overflow-y-auto py-6">{props.mode === "chat" ? props.messages.map((message) => <div key={message.id} className={`flex gap-3 ${message.role === "user" ? "justify-end" : "justify-start"}`}><div className={`max-w-[86%] rounded-2xl px-4 py-3 text-sm leading-6 ${message.role === "user" ? "bg-[#e7d9c7] text-[#171512]" : "border border-white/[.08] bg-white/[.035] text-white/78"}`}>{message.content}</div></div>) : visibleArtifacts.length ? <div className="grid gap-4 sm:grid-cols-2">{visibleArtifacts.map((artifact) => <div key={artifact.id} className="overflow-hidden rounded-2xl border border-white/[.08] bg-white/[.03]"><div className="aspect-square bg-black">{artifact.kind === "image" ? <img src={artifact.url} alt={artifact.prompt} className="h-full w-full object-cover" /> : <video src={artifact.url} controls className="h-full w-full object-cover" />}</div><div className="p-3"><div className="eyebrow">{artifact.model}</div><div className="mt-1 line-clamp-2 text-xs leading-5 text-white/55">{artifact.prompt}</div>{artifact.reference && <div className="mt-2 text-[10px] text-white/30">Used reference: {artifact.reference.name}</div>}<div className="mt-3 flex flex-wrap gap-2"><button onClick={() => { void downloadArtifact(artifact.url, `atlas-${artifact.kind}-${artifact.id}.${artifact.kind === "image" ? "png" : "mp4"}`); }} className="inline-flex items-center gap-1.5 rounded-lg border border-white/[.09] px-2.5 py-1.5 text-[10px] font-semibold text-white/55 hover:bg-white/[.05] hover:text-white"><Download size={12} /> Download</button>{artifact.kind === "image" && <button onClick={() => props.onUseArtifactReference(artifact)} className="inline-flex items-center gap-1.5 rounded-lg border border-[#c5b8ff]/25 bg-[#c5b8ff]/[.06] px-2.5 py-1.5 text-[10px] font-semibold text-[#c5b8ff] hover:bg-[#c5b8ff]/[.1]"><ImageIcon size={12} /> Use as reference</button>}</div></div></div>)}</div> : <div className="flex h-full min-h-[290px] flex-col items-center justify-center text-center"><div className="grid h-14 w-14 place-items-center rounded-2xl border border-white/[.1] bg-white/[.035] text-[#c5b8ff]">{props.mode === "image" ? <ImageIcon size={23} /> : <Clapperboard size={23} />}</div><div className="mt-5 text-sm font-semibold">Your {props.mode} board is empty</div></div>}
          ...
          <div className="rounded-2xl border border-white/[.1] bg-[#15171a] p-2">
            {mediaMode && props.reference && <div className="mb-2 flex items-center gap-3 rounded-xl border border-[#c5b8ff]/20 bg-[#c5b8ff]/[.05] p-2"><img src={props.reference.url} alt={props.reference.name} className="h-12 w-12 rounded-lg object-cover" /><div className="min-w-0 flex-1"><div className="eyebrow text-[#c5b8ff]">Reference</div><div className="mt-1 truncate text-xs text-white/65">{props.reference.name}</div></div><button onClick={props.onClearReference} className="rounded-lg p-2 text-white/35 hover:bg-white/[.06] hover:text-white" aria-label="Remove reference"><X size={14} /></button></div>}
            <textarea value={props.prompt} onChange={(event) => props.onPrompt(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); props.onSubmit(); } }} placeholder={props.mode === "chat" ? "Ask Atlas anything…" : props.reference ? "Describe what to change or how it should move…" : props.mode === "image" ? "Describe an image to create…" : "Describe a moving scene…"} rows={3} className="w-full resize-none bg-transparent px-2 py-1 text-sm leading-6 text-white outline-none placeholder:text-white/25" />
            <div className="flex items-center justify-between gap-3 px-1 pt-2"><div>{mediaMode && props.referenceSupported && <><input ref={uploadInput} type="file" accept="image/*" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) props.onUploadReference(file); event.currentTarget.value = ""; }} /><button disabled={props.uploading} onClick={() => uploadInput.current?.click()} className="inline-flex items-center gap-1.5 rounded-lg border border-white/[.09] px-2.5 py-2 text-[10px] font-semibold text-white/50 hover:bg-white/[.05] hover:text-white disabled:opacity-40">{props.uploading ? <LoaderCircle size={12} className="animate-spin" /> : <Upload size={12} />} {props.uploading ? "Uploading…" : props.reference ? "Replace reference" : "Add reference"}</button></>}</div><button disabled={!props.prompt.trim() || props.busy || props.uploading} onClick={props.onSubmit} className="flex items-center gap-2 rounded-xl bg-[#c5b8ff] px-3.5 py-2.5 text-xs font-bold text-[#17151f]">{props.busy ? <LoaderCircle size={14} className="animate-spin" /> : <Send size={14} />} {props.busy ? "Working" : props.mode === "chat" ? "Send" : "Generate"}</button></div>
          </div>
        </div>
        ...
        {mediaMode && <div className="rounded-[26px] border border-[#c5b8ff]/15 bg-[#c5b8ff]/[.05] p-5"><div className="text-xs font-bold">Single-image workflow</div><p className="mt-2 text-xs leading-5 text-white/45">Upload one image or reuse a generated image. Atlas switches automatically to the matching edit or image-to-video endpoint.</p></div>}
        <div className="rounded-[26px] border border-white/[.08] bg-[#0e1012] p-5"><div className="text-xs font-bold">Validated Atlas contract</div><p className="mt-2 text-xs leading-5 text-white/45">Only parameters supported by the selected model are sent and stored with the result.</p><a href="https://atlascloud.ai/docs" target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-1.5 text-[11px] font-semibold text-[#c5b8ff]">Atlas docs <ArrowUpRight size={12} /></a></div></div>
      </div>
    </section>
  </main>;
}

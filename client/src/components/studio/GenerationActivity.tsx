...
export default function GenerationActivity({ jobs, onCheck }: { jobs: GenerationJob[]; onCheck: (job: GenerationJob) => void }) {
  ...
  return <section aria-labelledby="generation-activity-title" className="rounded-[26px] border border-white/[.08] bg-[#0e1012] p-5">
    <div className="flex items-center justify-between gap-3">
      <div>
        <div className="eyebrow">Generation activity</div>
        <h2 id="generation-activity-title" className="mt-2 text-sm font-bold">Every submission stays visible</h2>
      </div>
    ...
    <div className="mt-4 space-y-3">
      {jobs.slice(0, 8).map((job) => {
        ...
        return <article key={job.id} className="rounded-xl border border-white/[.08] bg-white/[.025] p-3">
          ...
          <div className="mt-3 flex items-center gap-3">
            {canCheck && <button onClick={() => onCheck(job)} className="inline-flex items-center gap-1.5 text-[10px] font-semibold text-[#c5b8ff]"><RefreshCw size={11} /> Check now</button>}
            {job.resultUrl && <a href={job.resultUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-[10px] font-semibold text-white/50">Open result <ExternalLink size={11} /></a>}
          </div>
        </article>;
      })}
    </div>

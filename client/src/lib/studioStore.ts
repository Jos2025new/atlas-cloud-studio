...
function normalizeSession(value: unknown): StudioSession | null {
  ...
  if (typeof session.id !== "string" || typeof session.title !== "string" || typeof session.createdAt !== "string" || typeof session.updatedAt !== "string" || !["chat", "image", "video"].includes(String(session.mode)) || !Array.isArray(session.messages) || !Array.isArray(session.artifacts)) return null;
...
export function normalizeStore(value: unknown): StudioStore {
  ...
  const sessions = Array.isArray(candidate.sessions) ? candidate.sessions.map(normalizeSession).filter((session): session is StudioSession => Boolean(session)) : [];
...
export function updateSession(store: StudioStore, sessionId: string, updater: (session: StudioSession) => StudioSession): StudioStore { let changed = false; const sessions = store.sessions.map((session) => { if (session.id !== sessionId) return session; changed = true; return { ...updater(session), updatedAt: now() }; }); return changed ? { ...store, sessions } : store; }
...
export function activateSession(store: StudioStore, sessionId: string): StudioStore { return store.sessions.some((session) => session.id === sessionId) ? { ...store, activeSessionId: sessionId } : store; }
export function renameSession(store: StudioStore, sessionId: string, title: string): StudioStore { const clean = title.trim(); return clean ? updateSession(store, sessionId, (session) => ({ ...session, title: clean })) : store; }
export function deleteSession(store: StudioStore, sessionId: string): StudioStore { const sessions = store.sessions.filter((session) => session.id !== sessionId); const jobs = store.jobs.filter((job) => job.sessionId !== sessionId); if (!sessions.length) return createEmptyStore(); return { ...store, activeSessionId: store.activeSessionId === sessionId ? sessions[0].id : store.activeSessionId, sessions, jobs }; }
